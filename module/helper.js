...
export class EntitySheetHelper {
        ...
        for (let key of groupKeys) {
            ...
            Object.keys(group)
                ...
                .forEach((attr) => {
                    ...
                    group[attr]["isCheckbox"] =
                        group[attr]["dtype"] === "Boolean";
                    group[attr]["isResource"] =
                ...
                for (let i = 0; i < groups.length; i++) {
                    if (groups[i].value === val) {
                        ui.notifications.error(
                            game.i18n.localize("QUEST.NotifyAttrDuplicate") +
                                ` (${val})`
                        );
                        el.value = oldVal;
                        attrError = true;
    ...
    static validateGroup(groupName, document) {
        ...
        if (groups.includes(groupName)) {
            ui.notifications.error(
                game.i18n.localize("QUEST.NotifyGroupDuplicate") +
                    ` (${groupName})`
            );
            return false;
        }
        ...
        if (attributes.includes(groupName)) {
            ui.notifications.error(
                game.i18n.localize("QUEST.NotifyGroupAttrDuplicate") +
                    ` (${groupName})`
            );
            return false;
        }
        ...
        if (groupName.match(/[\s|\.]/i)) {
            ui.notifications.error(
                game.i18n.localize("QUEST.NotifyGroupAlphanumeric")
            );
            return false;
        }
        return true;
    ...
    static async createAttribute(event, app) {
        ...
        newKey.innerHTML = EntitySheetHelper.getAttributeHtml(
            htmlItems,
            nk,
            group
        );
        ...
        new Dialog({
            ...
            buttons: {
                confirm: {
                    ...
                    callback: async () => {
                        groupContainer.parentElement.removeChild(
                            groupContainer
                        );
                        await app._onSubmit(event);
                    }
